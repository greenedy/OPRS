...
public class PropertyDBHelper {
    ...
    public List<Property> findOwnerProperties(EntityManager em, SearchProperties searchProperty){
        String initQueryString = "SELECT p FROM Property p";
        String whereClauseConditions = "";
        Query query;
        
        
        if(!(null == searchProperty.getOwner())){ //If the NumberOfBathrooms field was used by the User
            whereClauseConditions += " p.owner = :owner";
        }else{
            //Select all Properties
            query = em.createQuery("SELECT p FROM Property p");
        }
